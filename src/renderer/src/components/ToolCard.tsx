...
const SOURCE_STYLE: Record<string, string> = {
  ...
  skill: 'bg-purple/10 text-purple',
  mcp: 'bg-info/10 text-info'
}
...
export default function ToolCard({ tc }: { tc: ToolCallView }): JSX.Element {
  ...
  return (
    <div className="my-1.5 overflow-hidden rounded-xl border border-border bg-panel/70 text-sm transition-colors hover:border-accent/30">
      <button
      ...
      >
        ...
        <span className="font-mono text-[13px] text-text">{tc.name}</span>
        <span className={`rounded-md px-1.5 py-0.5 text-[10px] ${SOURCE_STYLE[tc.source] || 'bg-surface text-muted'}`}>
      ...
      {open && (
        <div className="border-t border-border bg-surface/40 px-3 py-2.5">
          <div className="mb-1 text-[10.5px] font-medium uppercase tracking-wider text-muted">参数</div>
          <pre className="mb-2.5 overflow-x-auto rounded-lg border border-border bg-panel p-2.5 text-[12px] leading-relaxed text-text-secondary">
            {prettyArgs(tc.args)}
          </pre>
          {tc.result !== undefined && (
            <>
              ...
              <pre
                className={`overflow-x-auto whitespace-pre-wrap rounded-lg border border-border bg-panel p-2.5 text-[12px] leading-relaxed ${
                  tc.ok ? 'text-text-secondary' : 'text-danger'
                }`}
              >
