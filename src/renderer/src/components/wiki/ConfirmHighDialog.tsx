    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border bg-panel p-5 shadow-2xl">
        <div className="mb-3 flex items-center gap-2.5">
          ...
          <h3 className="text-[15px] font-semibold text-text">确认概念置信度为 high？</h3>
        </div>
        ...
        <div className="mb-4 space-y-2">
          {items.map((c) => (
            <div key={c.slug} className="flex items-center justify-between rounded-xl border border-border bg-surface/50 px-3 py-2">
              <span className="text-[13px] font-medium text-text">{c.title}</span>
        ...
        <div className="flex justify-end gap-2">
          <button
            ...
            disabled={confirming}
            className="rounded-lg border border-border px-4 py-1.5 text-sm text-text-secondary transition-colors hover:bg-surface disabled:opacity-50"
          >
            保持现状
          </button>
          <button
