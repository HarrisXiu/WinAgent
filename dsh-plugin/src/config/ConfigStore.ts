...
// 桌面版加密的 enc:v1: 配置无法解密，自动清空让用户重填。
 ...
 * 避免与动态工具列表重复、过期、互相矛盾。
 */
export const DEFAULT_PERSONA_PROMPT = `你是一个自主可控的 Windows 操作助手，同时管理着用户的个人知识库（LLM Wiki）。
 ...
 */
export const LEGACY_SYSTEM_PROMPT = `你是一个自主可控的 Windows 操作助手。你可以帮助用户：
...
【知识库工具】
search_knowledge_base: 全文搜索个人知识库（vault），返回标题、AI 摘要与正文片段，通常可直接据此回答
read_note: 读取知识库中指定笔记的完整内容与元数据（回答需要细节时使用）
...
当用户询问与你已存储的知识、文档、笔记相关的问题时，必须主动调用 search_knowledge_base 查找相关资料（返回已含标题、AI 摘要与正文片段，通常可直接据此回答）；命中不理想时换关键词（同义词/英文/缩写）重试一次；需要细节时用 read_note 阅读全文再作答。如果知识库中有相关信息，明确告诉用户这是来自其个人知识库的内容。
...
export function defaultConfig(): AppConfig {
  return {
    ...
    petPrompt: DEFAULT_PERSONA_PROMPT,
    vaultPath: '',
    knowledgeRag: {
      enabled: true,
      topK: 3,
...
export function getDataDir(): string {
  // DSH 插件数据目录：$DSH_HOME/winagent（可用 WINAGENT_DATA_DIR 覆盖）。
  // wiki、config.json、Logs、skills、mcp.json 都放在这里。
  const override = process.env.WINAGENT_DATA_DIR
  if (override) return path.resolve(override)
  const dshHome = process.env.DSH_HOME || path.join(os.homedir(), '.dsh')
  return path.join(dshHome, 'winagent')
}
    ...
    try {
      ...
      // 4) 迁移完成后 systemPrompt 置空（运行时不再读取）
      {
        ...
        const customSystem = typeof parsed.systemPrompt === 'string' &&
          parsed.systemPrompt.trim() && parsed.systemPrompt !== LEGACY_SYSTEM_PROMPT
        if (legacyDefault) {
          this.cfg.petPrompt = DEFAULT_PERSONA_PROMPT
          migrated = true
        } else if (customSystem && parsed.petPrompt === DEFAULT_PERSONA_PROMPT) {
