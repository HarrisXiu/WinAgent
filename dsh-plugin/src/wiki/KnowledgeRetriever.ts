...
export class KnowledgeRetriever {
  ...
  async retrieve(userInput: string): Promise<KnowledgeInjection | null> {
    ...
    if (hits.length === 0) {
      return {
        count: 0,
        text: [
          KNOWLEDGE_MARK + '提示】系统已在个人知识库中检索，未找到与本问题相关的内容。',
    ...
    const lines: string[] = [
      `${KNOWLEDGE_MARK}结果】（系统自动检索个人知识库所得，供回答参考，非用户输入；优先依据以下内容回答并注明来源笔记）`,
      ''
    ]
