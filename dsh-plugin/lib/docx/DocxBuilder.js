...
function latexToOmml(latex, display = true) {
    ...
    omml = omml.replace(/<m:t(?=[\s>])([^>]*)>(.*?)<\/m:t>/g, (_match, attrs, text) => {
        text = text.replace(/&amp;/g, '&');
...
function inlineRuns(text, opts = {}) {
    ...
    const rPr = () => {
        ...
        if (opts.italic)
            p.push('<w:i/>');
        if (half)
            p.push(`<w:sz w:val="${half}"/><w:szCs w:val="${half}"/>`);
...
function pPr(align, style, extra = '') {
    const parts = [];
    if (style)
...
function renderBlock(b) {
    switch (b.type) {
        ...
        case 'list':
            return b.items
                .map((it) => `<w:p>${pPr(undefined, undefined, `<w:numPr><w:ilvl w:val="0"/><w:numId w:val="${b.ordered ? 2 : 1}"/></w:numPr>`)}${inlineRuns(it)}</w:p>`)
                .join('');
        case 'table': {
...
function stylesXml(fontName, fontSize) {
    const half = Math.round(fontSize * 2);
    const heading = (lvl, sz) => `<w:style w:type="paragraph" w:styleId="Heading${lvl}"><w:name w:val="heading ${lvl}"/><w:basedOn w:val="Normal"/><w:pPr><w:keepNext/><w:outlineLvl w:val="${lvl - 1}"/><w:spacing w:before="${240 - lvl * 20}" w:after="120"/></w:pPr><w:rPr><w:b/><w:sz w:val="${sz}"/><w:szCs w:val="${sz}"/></w:rPr></w:style>`;
