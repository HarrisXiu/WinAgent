...
exports.WINAGENT_CONTRACT_MD = exports.MAX_CONTRACT_CHARS = void 0;
exports.truncateContract = truncateContract;
exports.extractContractSections = extractContractSections;
exports.readContract = readContract;
exports.readContractSections = readContractSections;
const fs_1 = require("fs");
const path_1 = __importDefault(require("path"));
/** 契约注入上限：默认契约全文约 3KB，8000 可完整容纳并留出用户扩展空间 */
exports.MAX_CONTRACT_CHARS = 8000;
/**
 * 行级截断契约文本：保证 `## ` 章节标题永远完整（不从中切断章节），
 * 超长正文行按字符截断。wikilink 铁律 / confidence 规则等章节因此不会半截丢失。
 * 返回值严格 ≤ maxChars（章节标题放不下剩余预算时停止，不产生溢出）。
 */
function truncateContract(md, maxChars = exports.MAX_CONTRACT_CHARS) {
 ...
 */
function extractContractSections(md, prioritySections, maxChars = exports.MAX_CONTRACT_CHARS) {
    ...
    const isPriority = (b) => prioritySections.some((k) => b.title.toLowerCase().includes(k.toLowerCase()));
...
async function readContract(vaultPath, maxChars = exports.MAX_CONTRACT_CHARS) {
    try {
        const raw = await fs_1.promises.readFile(path_1.default.join(vaultPath, 'CLAUDE.md'), 'utf-8');
...
async function readContractSections(vaultPath, prioritySections, maxChars = exports.MAX_CONTRACT_CHARS) {
    try {
        const raw = await fs_1.promises.readFile(path_1.default.join(vaultPath, 'CLAUDE.md'), 'utf-8');
...
exports.WINAGENT_CONTRACT_MD = `# WinAgent 知识库契约（CLAUDE.md）
