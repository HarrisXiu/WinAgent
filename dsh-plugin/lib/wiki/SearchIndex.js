...
class SearchIndex {
    ...
    tokenizeFields(fields) {
        ...
        for (const field of fields) {
            ...
            while ((m = wordRe.exec(field.text)) !== null) {
                ...
                if (word.length >= 2)
                    bump(word, field.weight);
                if (/[一-鿿]/.test(word)) {
                    if (word.length > 2) {
                        // 滑窗 bigram 覆盖子串命中
                        for (let i = 0; i < word.length - 1; i++)
                            bump(word.slice(i, i + 2), field.weight);
                    }
                    // unigram：单字查询兜底（低权重 + IDF 自然压制常见字）；单字 CJK 串也入索引
                    for (const ch of word)
                        bump(ch, field.weight * UNIGRAM_DISCOUNT);
                }
            }
        }
        return weights;
     ...
     */
    indexNote(note, content, summary, extra) {
        // 移除旧条目
    ...
    generateSnippet(doc, query) {
        ...
        if (bestIdx === -1 && doc.summary) {
            ...
            if (bestIdx !== -1)
                return this.sliceWindow(doc.content, bestIdx);
        }
        if (bestIdx === -1)
            return (doc.summary || doc.content).slice(0, 200);
        return this.sliceWindow(target, bestIdx);
    }
    sliceWindow(content, idx) {
