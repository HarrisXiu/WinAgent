...
exports.runReflect = runReflect;
exports.runStage0Check = runStage0Check;
...
async function runLint(vm) {
    ...
    const report = [`# Lint 报告 ${today}`, ''];
    const issues = [];
    const modifiedRawFiles = [];
    // 收集 wiki/ 下所有页面文件（排除系统文件、templates、outputs）
    const allFiles = [];
    const collect = async (dir, rel) => {
        try {
            const entries = await fs_1.promises.readdir(dir, { withFileTypes: true });
            for (const e of entries) {
                if (e.isDirectory()) {
                    await collect(path_1.default.join(dir, e.name), `${rel}/${e.name}`);
                }
    ...
    await collect(path_1.default.join(vault, 'wiki'), 'wiki');
    const pages = allFiles.filter((f) => !vm.isSystemFile(f) && !f.startsWith('wiki/templates/') && !f.startsWith('wiki/outputs/'));
    ...
    for (const f of allFiles) {
        ...
        pageIds.add(f);
        pageIds.add(path_1.default.basename(f).replace(/\.md$/, ''));
    }
    ...
    for (const f of systemPages) {
        ...
        systemIds.add(f);
        systemIds.add(path_1.default.basename(f).replace(/\.md$/, ''));
    }
    // 检查 1: frontmatter 合法性
    const fmBad = [];
    for (const f of pages) {
        try {
            const raw = await fs_1.promises.readFile(path_1.default.join(vault, f), 'utf-8');
    ...
    // 检查 3: Index 一致性
    const indexRaw = await fs_1.promises.readFile(path_1.default.join(vault, 'wiki/index.md'), 'utf-8').catch(() => '');
    ...
    for (const f of pages) {
        try {
            ...
            if (bodyLen > 0 && bodyLen < 100)
                issues.push(`⚠ 检查4 stub: ${f} 正文仅 ${bodyLen} 字`);
        }
        catch { /* skip */ }
    }
    // 检查 5: 近重复概念（slug Jaccard > 0.7）
    const conceptDir = path_1.default.join(vault, 'wiki/concepts');
    const conceptSlugs = await fs_1.promises.readdir(conceptDir).catch(() => []);
    ...
    for (let i = 0; i < mdSlugs.length; i++) {
        for (let j = i + 1; j < mdSlugs.length; j++) {
            ...
            if (sim > 0.7)
                issues.push(`⚠ 检查5 近重复: "${mdSlugs[i]}" 与 "${mdSlugs[j]}" Jaccard=${sim.toFixed(2)}`);
        }
    }
    // 检查 6: SHA-256 完整性（source 页 raw_sha256 vs raw 文件）
    const sourceDir = path_1.default.join(vault, 'wiki/sources');
    const sourceFiles = await fs_1.promises.readdir(sourceDir).catch(() => []);
    for (const f of sourceFiles.filter((f) => f.endsWith('.md'))) {
        try {
            const raw = await fs_1.promises.readFile(path_1.default.join(sourceDir, f), 'utf-8');
            const parsed = (0, gray_matter_1.default)(raw);
            const fm = parsed.data;
            const rawFile = fm.raw_file;
            const expected = fm.raw_sha256;
            if (typeof rawFile === 'string' && typeof expected === 'string') {
                const buf = await fs_1.promises.readFile(path_1.default.join(vault, rawFile)).catch(() => null);
                if (buf) {
                    const actual = (0, crypto_1.createHash)('sha256').update(buf).digest('hex');
                    if (actual !== expected) {
    ...
    const staleThreshold = { high: 90, medium: 180, low: 365 };
    ...
    await fs_1.promises.writeFile(path_1.default.join(vault, reportPath), gray_matter_1.default.stringify(reportBody, { type: 'lint-report', date: today, 'graph-excluded': true }), 'utf-8');
...
// ==================== MERGE（去重合并） ====================
async function runMerge(vm, keep, remove, area) {
    ...
    const keepAbs = path_1.default.join(vault, keepPath);
    const removeAbs = path_1.default.join(vault, removePath);
    try {
        await fs_1.promises.access(removeAbs);
    }
    catch {
    ...
    let keepRaw = '';
    try {
        keepRaw = await fs_1.promises.readFile(keepAbs, 'utf-8');
    }
    catch {
        return { ok: false, reportPath: '', summary: '', error: `保留页面不存在: ${keepPath}` };
    }
    const keepParsed = (0, gray_matter_1.default)(keepRaw);
    const removeParsed = (0, gray_matter_1.default)(await fs_1.promises.readFile(removeAbs, 'utf-8'));
    const keepFm = keepParsed.data;
    const removeFm = removeParsed.data;
    // 1. aliases 并集
    ...
    const mergeSection = (body, section) => {
    ...
    const collectAll = async () => {
        ...
        const walk = async (dir) => {
            ...
            for (const e of entries) {
                if (e.isDirectory()) {
                    await walk(path_1.default.join(dir, e.name));
                }
                else if (e.name.endsWith('.md')) {
                    files.push(path_1.default.join(dir, e.name));
                }
            }
        };
        await walk(path_1.default.join(vault, 'wiki'));
        return files;
    };
    ...
    for (const f of wikiFiles) {
        if (f === removeAbs)
            continue;
        const raw = await fs_1.promises.readFile(f, 'utf-8');
        if (raw.includes(`[[${remove}]]`)) {
            const updated = raw.split(`[[${remove}]]`).join(`[[${keep}]]`);
            await fs_1.promises.writeFile(f, updated, 'utf-8');
        }
    }
    // 4. 写回保留页 + 被合并页替换为 redirect
    await fs_1.promises.writeFile(keepAbs, gray_matter_1.default.stringify(newBody, keepFm), 'utf-8');
    await fs_1.promises.writeFile(removeAbs, gray_matter_1.default.stringify(`> 此页面已被合并到 [[${keep}]]。`, { redirect: `wiki/${area}/${keep}` }), 'utf-8');
...
async function runReflect(vm, store, signal) {
    ...
    const stage0Lines = stage0.map((s) => `- ${s.path}: ${s.status === 'ok' ? '✅ 通过' : `⚠ ${s.reason}`}`);
    ...
    const contract = await (0, contract_1.readContract)(vault);
...
反向检验要求：在生成结论前主动寻找与候选结论相矛盾的证据。${echoChamberHint}${degradeHint}${contractRule}`
    ...
    const synthesisBody = [
        ...
        `## Contradictions`,
        ``,
        (parsed.contradictions || []).map((c) => `- ${c}`).join('\n') || '（无）',
        ``,
        `## Gap Analysis`,
        ``,
    ...
    await fs_1.promises.mkdir(path_1.default.join(vault, 'wiki/synthesis'), { recursive: true });
    await fs_1.promises.writeFile(path_1.default.join(vault, synthesisPath), gray_matter_1.default.stringify(synthesisBody, { type: 'synthesis', title: `综合分析 ${today}`, date: today, source_count: sources.length, confidence: 'medium' }), 'utf-8');
    // 写入 gap 报告
    const gapPath = `wiki/outputs/gap-report-${today}.md`;
    await fs_1.promises.mkdir(path_1.default.join(vault, 'wiki/outputs'), { recursive: true });
    await fs_1.promises.writeFile(path_1.default.join(vault, gapPath), gray_matter_1.default.stringify(`# Gap Report ${today}\n\n## 孤立概念\n\n${(parsed.orphans || []).map((o) => `- ${o}`).join('\n') || '（无）'}\n\n## 内容空白\n\n${(parsed.gaps || []).map((g) => `- ${g}`).join('\n') || '（无）'}\n\n## Stage 0 反向检验\n\n${stage0Lines.join('\n') || '（无）'}\n`, { type: 'gap-report', date: today, 'graph-excluded': true }), 'utf-8');
    // 更新 overview
...
async function runStage0Check(vm) {
    const vault = vm.getVaultPath();
    const sourceDir = path_1.default.join(vault, 'wiki/sources');
    const files = await fs_1.promises.readdir(sourceDir).catch(() => []);
    const results = [];
...
async function collectPageBriefs(vault, area, maxLen) {
    const dir = path_1.default.join(vault, 'wiki', area);
    const files = await fs_1.promises.readdir(dir).catch(() => []);
    const briefs = [];
    for (const f of files.filter((f) => f.endsWith('.md'))) {
        try {
            const raw = await fs_1.promises.readFile(path_1.default.join(dir, f), 'utf-8');
            const parsed = (0, gray_matter_1.default)(raw);
            const fm = parsed.data;
...
async function runQuery(vm, searchIndex, store, query, signal) {
    ...
    for (let i = 0; i < readCount; i++) {
        try {
            ...
            contextBlocks.push(`【候选 ${i + 1}】路径: ${note.path}（标题: ${note.title}）\n${note.rawBody.slice(0, 2500)}`);
        }
        catch { /* skip */ }
    }
