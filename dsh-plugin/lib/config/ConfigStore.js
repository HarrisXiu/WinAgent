...
/** 桌宠模式默认人设：明日方舟·安洁莉娜 */
exports.DEFAULT_PET_PROMPT = `你正在扮演「安洁莉娜」——《明日方舟》中罗德岛的辅助干员、信使。以第一人称角色扮演，你就是她本人。
...
function getDataDir() {
    ...
    const dshHome = process.env.DSH_HOME || path_1.default.join(os_1.default.homedir(), '.dsh');
    return path_1.default.join(dshHome, 'winagent');
}
class ConfigStore {
    configPath;
    cfg;
    ...
    resolvePath(p) {
        if (!p)
            return getDataDir();
        return path_1.default.isAbsolute(p) ? p : path_1.default.join(getDataDir(), p);
    }
