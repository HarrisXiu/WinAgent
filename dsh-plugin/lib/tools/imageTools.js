...
exports.imageTools = [
    {
        schema: {
            ...
            description: '为图像生成工具（Midjourney / Stable Diffusion / 即梦AI / DALL·E 等）生成可直接复制使用的绘图提示词（Prompt）。' +
            ...
            parameters: {
                ...
                properties: {
                    ...
                    style: {
                        type: 'string',
                        description: '期望风格（可选）：写实/照片/动漫/二次元/赛博朋克/水彩/油画/素描/3D/pixar/像素/极简/国风/科幻/梦幻/可爱'
                    },
        ...
        async run(a) {
            const request = (0, types_1.str)(a.request).trim();
            if (!request)
                throw new Error('request 不能为空');
            const style = (0, types_1.str)(a.style).trim();
            const styleKw = STYLE_KEYWORDS[style] || STYLE_KEYWORDS['写实'];
            const ratio = RATIO_PARAM[(0, types_1.str)(a.aspect_ratio).trim()] || RATIO_PARAM['1:1'];
            ...
            return [
                ...
                `- 构图：专业构图，主体居中偏三分线`,
                `- 比例：${(0, types_1.str)(a.aspect_ratio).trim() || '1:1'}（${ratio}）`,
                ``,
                `💡 使用建议：`,
