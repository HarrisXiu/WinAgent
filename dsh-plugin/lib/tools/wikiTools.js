...
exports.createWikiTools = createWikiTools;
    ...
    return [
        {
            ...
            async run(a) {
                const results = searchIndex.search((0, types_1.str)(a.query), (0, types_1.num)(a.limit, 10));
                if (results.length === 0)
        ...
        },
        {
            schema: {
                name: 'read_note',
                description: '读取知识库中某篇笔记的完整内容（含元数据：标签、创建时间、AI 摘要等）。建议先用 search_knowledge_base 找到相关笔记路径，需要细节时再用此工具获取全文。',
                parameters: {
                    type: 'object',
                    properties: {
            ...
            async run(a) {
                const note = await vaultManager.readNote((0, types_1.str)(a.path));
                return [
                    `# ${note.title}`,
                    `标签: ${note.tags.join(', ') || '无'}`,
        ...
        },
        {
            schema: {
                name: 'list_notes',
                description: '列出知识库的结构（LLM Wiki 分层：raw 原始文件区 + wiki 编译知识区）。用于浏览知识库的组织，了解有哪些可用的知识。',
                parameters: {
                    type: 'object',
                    properties: {}
                }
            ...
            async run(_a) {
                ...
                if (notes.length === 0)
                    return '知识库为空，还没有任何笔记。';
                const lines = ['知识库（LLM Wiki 模式）:', ''];
                const render = (items, prefix) => {
                    for (const n of items) {
                        if (n.kind === 'folder') {
                            lines.push(`${prefix}📁 ${n.title}/`);
                            if (n.children)
                                render(n.children, prefix + '  ');
                        }
                        else {
                ...
                if (rawItems?.children)
                    render(rawItems.children, '  ');
                lines.push('', '📚 wiki/ — 编译知识（LLM 维护，检索此区域）:');
                if (wikiItems?.children)
                    render(wikiItems.children, '  ');
        ...
        },
        {
            schema: {
                name: 'read_raw_file',
                description: '读取知识库 raw 层（原始剪藏文件）的完整内容，用于查看来源原文。知识库索引建立在 wiki 编译层上，需要溯源原文时用此工具。',
                parameters: {
                    type: 'object',
                    properties: {
            ...
            async run(a) {
                const rel = (0, types_1.str)(a.path);
                if (!rel.startsWith('raw/'))
                    return '只允许读取 raw/ 目录下的文件';
        ...
        },
        {
            schema: {
                name: 'add_question',
                description: '记录一个开放问题到知识库的 QUESTIONS.md（问题队列）。当用户想搞清楚某个问题、希望在后续摄入中自动匹配答案时使用。',
                parameters: {
                    type: 'object',
                    properties: {
            ...
            async run(a) {
                const q = (0, types_1.str)(a.question);
                if (!q)
                    return '问题不能为空';
        ...
        },
        {
            schema: {
                ...
                description: '将高价值的查询答案/分析结果持久化到 wiki/outputs/（知识库输出层）。当回答用户基于知识库的问题且答案有复用价值时使用，答案不会被对话冲走。',
                parameters: {
                    type: 'object',
                    properties: {
            ...
            async run(a) {
                const title = (0, types_1.str)(a.title);
                ...
                const body = `# ${title}\n\n${content}\n`;
                await fs_1.promises.writeFile(path_1.default.join(vaultManager.getVaultPath(), outPath), gray_matter_1.default.stringify(body, fm), 'utf-8');
        ...
        },
        {
            ...
            async run(_a) {
                const r = await (0, WorkflowService_1.runLint)(vaultManager);
        ...
        },
        {
            schema: {
                ...
                description: '合并两个重复的知识库页面（同语言或跨语言）。主 slug 保留，被合并页面的 wikilinks 全部更新，被合并文件替换为重定向文件。执行前必须先与用户确认合并方案（绝不自动合并）。',
                parameters: {
                    type: 'object',
                    properties: {
        ...
        },
        {
            ...
            async run(_a) {
                const r = await (0, WorkflowService_1.runReflect)(vaultManager, store);
