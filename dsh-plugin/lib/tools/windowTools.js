...
exports.windowTools = [
    {
        ...
        async run(a) {
            const title = (0, types_1.str)(a.title).toLowerCase();
            const proc = (0, types_1.str)(a.process).toLowerCase().replace(/\.exe$/, '');
            const r = await (0, powershell_1.runPowerShell)(`Get-Process | Where-Object {$_.MainWindowTitle -ne ''} | Select-Object Id,ProcessName,MainWindowTitle,@{N='H';E={$_.MainWindowHandle.ToInt64()}} | ConvertTo-Json -Compress`);
            let arr = [];
            try {
            ...
            const rows = arr
                .filter((w) => (!title || String(w.MainWindowTitle).toLowerCase().includes(title)) &&
    ...
    },
    {
        ...
        dangerous: true,
        async run(a) {
            const stateMap = { minimize: 6, maximize: 3, restore: 9 };
            const sw = stateMap[(0, types_1.str)(a.state)] ?? 9;
            const sel = a.title
                ? `Where-Object {$_.MainWindowTitle -like '*'+${(0, powershell_1.psQuote)((0, types_1.str)(a.title))}+'*'}`
                : `Where-Object {$_.ProcessName -like '*'+${(0, powershell_1.psQuote)((0, types_1.str)(a.process).replace(/\.exe$/, ''))}+'*'}`;
...
if($p){[W]::ShowWindow($p.MainWindowHandle,${sw})|Out-Null;[W]::SetForegroundWindow($p.MainWindowHandle)|Out-Null;Write-Output 'ok'}else{Write-Output 'notfound'}`;
            ...
            if (r.stdout.includes('ok'))
                return `已${(0, types_1.str)(a.state)}窗口`;
            if (r.stdout.includes('notfound'))
                return '未找到匹配窗口';
