...
async function statSafe(p) {
    try {
        return await fs_1.promises.stat(p);
    }
    catch {
        return null;
    }
}
async function walkTree(dir, depth, maxDepth, entries, maxEntries, prefix = '') {
    if (depth > maxDepth || entries.length >= maxEntries)
        return;
    let items;
    try {
        items = await fs_1.promises.readdir(dir);
    }
    catch {
        return;
    }
...
exports.fileTools = [
    {
        ...
        async run(a) {
            const dir = (0, types_1.str)(a.path);
            if ((0, types_1.bool)(a.recursive)) {
                const entries = [];
                await walkTree(dir, 1, (0, types_1.num)(a.max_depth, 5), entries, (0, types_1.num)(a.max_entries, 500));
                return entries.length ? entries.join('\n') : '（空目录）';
            }
            const items = await fs_1.promises.readdir(dir);
            const lines = [];
            for (const name of items) {
                const st = await statSafe(path_1.default.join(dir, name));
                if (!st)
                    continue;
    ...
    },
    {
        ...
        async run(a) {
            const content = await fs_1.promises.readFile((0, types_1.str)(a.path), 'utf-8');
            const offset = (0, types_1.num)(a.offset, 0);
            const length = (0, types_1.num)(a.length, 20000);
    ...
    },
    {
        ...
        dangerous: true,
        async run(a) {
            const p = (0, types_1.str)(a.path);
            await fs_1.promises.mkdir(path_1.default.dirname(p), { recursive: true });
            if ((0, types_1.bool)(a.append))
                await fs_1.promises.appendFile(p, (0, types_1.str)(a.content), 'utf-8');
            else
                await fs_1.promises.writeFile(p, (0, types_1.str)(a.content), 'utf-8');
            return `已写入: ${p}`;
        }
    },
    {
        ...
        async run(a) {
            const p = (0, types_1.str)(a.path);
            const content = await fs_1.promises.readFile(p, 'utf-8');
            const oldStr = (0, types_1.str)(a.old_string);
            const newStr = (0, types_1.str)(a.new_string);
            if ((0, types_1.bool)(a.replace_all)) {
                const count = content.split(oldStr).length - 1;
                if (count === 0)
                    throw new Error('未找到匹配的 old_string');
                await fs_1.promises.writeFile(p, content.split(oldStr).join(newStr), 'utf-8');
            ...
            await fs_1.promises.writeFile(p, content.slice(0, first) + newStr + content.slice(first + oldStr.length), 'utf-8');
            return `已替换: ${p}`;
        }
    },
    {
        ...
        async run(a) {
            const p = (0, types_1.str)(a.path);
            let content = await fs_1.promises.readFile(p, 'utf-8');
            const edits = Array.isArray(a.edits) ? a.edits : [];
            for (const [i, e] of edits.entries()) {
                const oldStr = (0, types_1.str)(e.old_string);
                const newStr = (0, types_1.str)(e.new_string);
                if ((0, types_1.bool)(e.replace_all)) {
                    if (!content.includes(oldStr))
                        throw new Error(`edit[${i}] 未找到匹配`);
    ...
    },
    {
        ...
        async run(a) {
            ...
            if (st.isDirectory())
                await fs_1.promises.rm(p, { recursive: (0, types_1.bool)(a.recursive), force: false });
            else
    ...
    },
    {
        ...
        async run(a) {
            const src = (0, types_1.str)(a.source);
            const dst = (0, types_1.str)(a.destination);
            await fs_1.promises.mkdir(path_1.default.dirname(dst), { recursive: true });
            await fs_1.promises.copyFile(src, dst, (0, types_1.bool)(a.overwrite) ? 0 : fs_1.promises.constants.COPYFILE_EXCL);
            return `已复制: ${src} → ${dst}`;
        }
    },
    {
    ...
    },
    {
        ...
        async run(a) {
            const dir = (0, types_1.str)(a.directory);
            const pattern = (0, types_1.str)(a.pattern).replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.');
            const re = new RegExp('^' + pattern + '$', 'i');
            const results = [];
            const recursive = a.recursive === undefined ? true : (0, types_1.bool)(a.recursive);
            const max = (0, types_1.num)(a.max_results, 50);
            ...
            const walk = async (d) => {
                ...
                let items;
                try {
                    items = await fs_1.promises.readdir(d);
                }
                catch {
                    return;
                }
                for (const name of items) {
                    if (results.length >= max)
                        return;
                    const full = path_1.default.join(d, name);
                    const s = await statSafe(full);
                    if (!s)
                        continue;
                    if (s.isDirectory()) {
                        if (recursive && !SKIP_DIRS.has(name))
                            await walk(full);
                    }
    ...
    },
    {
        ...
        async run(a) {
            ...
            if (!st)
                return '路径不存在: ' + p;
            if (st.isDirectory()) {
                const items = await fs_1.promises.readdir(p);
                let files = 0;
                let dirs = 0;
                for (const name of items) {
                    const s = await statSafe(path_1.default.join(p, name));
                    if (s?.isDirectory())
                        dirs++;
                    else
                        files++;
    ...
    },
    {
        ...
        async run(a) {
            await fs_1.promises.mkdir((0, types_1.str)(a.path), { recursive: true });
            return `已创建目录: ${(0, types_1.str)(a.path)}`;
        }
    },
    {
        schema: {
        ...
        async run(a) {
            const re = new RegExp((0, types_1.str)(a.pattern), (0, types_1.bool)(a.ignore_case) ? 'i' : '');
            const globRe = a.glob
                ? new RegExp('^' + (0, types_1.str)(a.glob).replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.') + '$', 'i')
                : null;
            const max = (0, types_1.num)(a.max_matches, 100);
            const out = [];
            const searchFile = async (f) => {
                if (out.length >= max)
                    return;
                if (globRe && !globRe.test(path_1.default.basename(f)))
                    return;
                let text;
                try {
                    text = await fs_1.promises.readFile(f, 'utf-8');
                }
                catch {
                    return;
                }
            ...
            const st = await statSafe((0, types_1.str)(a.path));
            if (!st)
                return '路径不存在';
            if (st.isFile())
                await searchFile((0, types_1.str)(a.path));
            else {
                const walk = async (d) => {
                    if (out.length >= max)
                        return;
                ...
                await walk((0, types_1.str)(a.path));
            }
