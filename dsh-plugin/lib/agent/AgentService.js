...
class AgentService {
    ...
    systemMessage(cfg) {
        ...
        const capability = [
            ...
            '- 【知识库检索】用户问题涉及"我的知识库/笔记/wiki/我收集的资料"等时，必须主动调用 search_knowledge_base 检索（返回已含标题、AI 摘要与正文片段，通常可直接据此回答）；命中不理想时换关键词（同义词/英文/缩写）重试一次；需要细节时用 read_note 读全文；回答时注明来源笔记标题，核心结论溯源到 wiki/sources/ 具体来源页，来源矛盾时显式标注分歧',
            ...
            for (let round = 1; round <= MAX_ROUNDS; round++) {
                ...
                try {
                    ...
                    if (sentImages && !downgraded && isImageUnsupportedError(msg)) {
                        ...
                        if (idx >= 0)
                            this.history[idx] = { role: 'user', content: rebuilt.content };
                        continue;
                    }
                    throw e;
                }
                ...
                for (const call of result.toolCalls) {
                    ...
                    Logger_1.Logger.info(`[TOOL RESULT] ${call.name}: ${toolResult.slice(0, 200)}`);
