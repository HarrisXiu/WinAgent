...
function detectVision(provider) {
 ...
 */
function resolveVisionProvider(cfg, main) {
    ...
    if (!model)
        return undefined;
    if (base.id === main.id && model === main.model)
        return undefined;
...
function isImageUnsupportedError(msg) {
    ...
    return (m.includes('support image input') ||
 ...
 */
function isFileUnsupportedError(msg) {
    const m = msg.toLowerCase();
    return (m.includes('file_data') ||
 ...
 */
function appendKnowledge(content, knowledgeText) {
    if (!knowledgeText)
        return content;
    ...
    if (Array.isArray(content)) {
        ...
        if (textIdx >= 0) {
            parts[textIdx] = { type: 'text', text: `${parts[textIdx].text}\n\n${knowledgeText}` };
        }
        else {
...
class AgentService {
    ...
    reportUsage(last, cb) {
        ...
        Logger_1.Logger.info(`[Usage] 本次 prompt=${last.prompt} completion=${last.completion} total=${last.total}` +
    ...
    systemMessage(cfg) {
        ...
        if (caps.pandoc)
            docCaps.push('pandoc 可用：Word 生成自动走 pandoc 引擎');
    ...
    extractToolImages(text) {
        const images = [];
        const clean = text.replace(TOOL_IMG_RE, (_m, url) => {
            images.push(url);
            return '';
        });
     ...
     */
    async stripHistoryImages(cfg, provider, userInput, cb) {
        ...
        for (let i = 0; i < this.history.length; i++) {
            ...
            for (const p of msg.content) {
                if (p.type === 'text')
                    ...
                    images.push({ name: `image-${i + 1}-${images.length + 1}`, dataUrl: p.image_url.url });
                else if (p.type === 'file')
                    keptFiles.push(p);
            }
     ...
     */
    async describeImages(cfg, visionProvider, images, userInput, cb) {
        ...
        for (const img of images) {
            ...
            try {
                ...
                this.reportUsage(res.usage ?? {
                    prompt: estimateText(prompt) + ContextManager_1.IMAGE_TOKEN_COST,
                    completion: estimateText(text),
                    total: estimateText(prompt) + ContextManager_1.IMAGE_TOKEN_COST + estimateText(text),
                    estimated: true
                }, cb);
                results.push(`\n[图片: ${img.name}]（由视觉模型 ${visionProvider.model} 识别）\n${text}`);
                cb.onEvent({ type: 'vision', status: 'done', model: visionProvider.model, text });
                Logger_1.Logger.info(`[VisionAssist] "${img.name}" 识别完成，${text.length} 字符`);
            }
            catch (e) {
                const msg = e instanceof Error ? e.message : String(e);
                results.push(`\n[图片: ${img.name}] 视觉模型识别失败: ${msg}`);
                cb.onEvent({ type: 'vision', status: 'error', model: visionProvider.model, text: msg });
                Logger_1.Logger.error(`[VisionAssist] "${img.name}" 识别失败: ${msg}`);
            }
        }
        return results;
    }
     ...
     */
    async buildUserContent(cfg, provider, userInput, attachments, cb, forceNoVision = false) {
        let userContent = userInput;
        let multipart = false;
        let hasFiles = false;
        if (attachments && attachments.length > 0) {
            // PDF 附件：文件直传（不依赖 vision，文本模型也可读文档）；被拒/超限/禁用时降级为路径提示
            const isPdf = (a) => a.isPdf === true || a.mime === 'application/pdf' || /\.pdf$/i.test(String(a.name || ''));
            ...
            for (const att of pdfs) {
                ...
                if (!dataUrl && att.path) {
                    try {
                        const buf = await fs_1.promises.readFile(att.path);
            ...
            if (others.length > 0) {
                ...
                if (!forceNoVision) {
                    Logger_1.Logger.info(`[Vision] model="${provider.model}" supportsVision=${provider.supportsVision} auto=${supportsVision}`);
                }
                if (supportsVision) {
                    ...
                    if (visionProvider && sendable.length > 0) {
                        ...
                        const descs = await this.describeImages(cfg, visionProvider, sendable.map((a) => ({ name: a.name, dataUrl: a.dataUrl })), userInput, cb);
    ...
    async process(userInput, cb, attachments) {
        const cfg = this.store.get();
        const provider = this.store.activeProvider();
        const ctx = new ContextManager_1.ContextManager(cfg);
        ...
        if (this.knowledge && userInput.trim()) {
            try {
                ...
                if (inj)
                    cb.onEvent({ type: 'knowledge', query: userInput.slice(0, 50), count: inj.count });
            }
            catch (e) {
                Logger_1.Logger.warn(`[RAG] 知识库检索失败（已跳过）: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        this.history.push({ role: 'user', content: appendKnowledge(built.content, knowledgeText) });
        Logger_1.Logger.info(`[USER] ${userInput}${attachments ? ` (+${attachments.length} 附件)` : ''}`);
        try {
            for (let round = 1; round <= MAX_ROUNDS; round++) {
                // 上下文压缩
                if (ctx.needsCompact(this.history)) {
                    const before = (0, ContextManager_1.estimateTokens)(this.history);
                    this.history = await ctx.compact(provider, this.history);
                    const after = (0, ContextManager_1.estimateTokens)(this.history);
                    cb.onEvent({ type: 'compact', before, after });
                    Logger_1.Logger.info(`[Compact] ${before} → ${after} tokens`);
                }
                ...
                let result;
                try {
                    result = await (0, OpenAIClient_1.chatStream)(provider, messages, {
                    ...
                    if (sentImages && !downgraded && isImageUnsupportedError(msg)) {
                        downgraded = true;
                        sentImages = false;
                        Logger_1.Logger.info(`[Vision] 模型 "${provider.model}" 实际不接受图片输入，降级为视觉辅助/文本描述重试`);
                        cb.onEvent({
                            type: 'vision',
                            status: 'error',
                    ...
                    if (sentFiles && !filesDowngraded && isFileUnsupportedError(msg)) {
                        ...
                        sentFiles = false;
                        Logger_1.Logger.info(`[FilePass] 模型 "${provider.model}" 不接受 PDF 文件输入，降级为工具读取提示重试`);
                        cb.onEvent({
                            type: 'vision',
                            status: 'error',
                            model: provider.model,
                ...
                this.reportUsage(result.usage ?? {
                    prompt: (0, ContextManager_1.estimateTokens)(messages),
                    completion: estimateText(completionText),
                    total: (0, ContextManager_1.estimateTokens)(messages) + estimateText(completionText),
                    estimated: true
                }, cb);
                ...
                for (const call of result.toolCalls) {
                    ...
                    Logger_1.Logger.info(`[TOOL CALL] ${call.name} ${call.arguments}`);
                    let args = {};
                    try {
                    ...
                    Logger_1.Logger.info(`[TOOL RESULT] ${call.name}: ${shown.slice(0, 200)}`);
                    this.history.push({ role: 'tool', content: shown, tool_call_id: call.id, name: call.name });
                    if (toolImages.length > 0)
                        roundImages.push(...toolImages);
                }
                ...
                if (roundImages.length > 0) {
                    ...
                    if (capped.length < roundImages.length) {
                        Logger_1.Logger.warn(`[ToolImage] 本轮工具图片 ${roundImages.length} 张超出上限，仅注入前 ${capped.length} 张`);
                    }
                    if (detectVision(provider)) {
                        ...
                        const visionProvider = resolveVisionProvider(cfg, provider);
                        if (visionProvider) {
                            const descs = await this.describeImages(cfg, visionProvider, capped.map((url, i) => ({ name: `tool-image-${i + 1}`, dataUrl: url })), userInput, cb);
                            this.history.push({
                                role: 'user',
            ...
            Logger_1.Logger.error(`[Agent] ${msg}`);
        }
        finally {
            this.abort = null;
        }
