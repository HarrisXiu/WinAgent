...
function contentTokens(c) {
    ...
    for (const p of c) {
        if (p.type === 'text')
            t += Math.ceil(p.text.length / 3);
        else if (p.type === 'file')
            t += 4000; // PDF 文件直传按固定开销估算（实际随页数浮动）
        else
            t += exports.IMAGE_TOKEN_COST; // 图片按固定开销估算
    }
    return t;
}
function estimateTokens(messages) {
    ...
    for (const m of messages) {
        ...
        if (m.reasoning_content)
            total += Math.ceil(m.reasoning_content.length / 3);
        if (m.tool_calls)
            total += Math.ceil(JSON.stringify(m.tool_calls).length / 3);
        total += 4; // 每条消息的固定开销
    }
    return total;
}
 ...
 */
function safeSplitIndex(history, desired) {
    let idx = desired;
    while (idx > 0 && history[idx].role === 'tool')
        idx--;
    return idx;
}
...
class ContextManager {
     ...
     */
    async compact(provider, history) {
        ...
        const trimmed = history.map((m, i) => {
            ...
            if (m.role === 'user' && typeof m.content === 'string' && m.content.includes(KnowledgeRetriever_1.KNOWLEDGE_MARK)) {
                return { ...m, content: m.content.slice(0, m.content.indexOf(KnowledgeRetriever_1.KNOWLEDGE_MARK)).trimEnd() + '\n…[知识库检索内容已省略]' };
            }
            ...
            if (Array.isArray(m.content)) {
                ...
                const notes = [];
                if (imgCount > 0)
                    notes.push(`[${imgCount} 张图片已省略]`);
                if (fileCount > 0)
                    notes.push(`[${fileCount} 个 PDF 文件已省略，路径见文本]`);
